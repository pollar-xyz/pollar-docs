...
The primary hook. Returns a single object with all Pollar functionality — authentication, wallet, payments, and history. Use this if you prefer working with a single hook.
...
For more focused usage, Pollar also exports individual hooks per domain.
...
const { wallet, activateWallet, fund, loading } = usePollarWallet();
```

| Property         | Type                                       | Description                                                                                     |
| ---------------- | ------------------------------------------ | ----------------------------------------------------------------------------------------------- |
| `wallet`         | `Wallet \| null`                           | Current user's wallet                                                                           |
| `activateWallet` | `() => Promise<void>`                      | Triggers wallet activation — should call your backend which then calls `POST /wallets/activate` |
...
The button and modal styles are customizable from **Dashboard → Branding** — no code changes needed.
